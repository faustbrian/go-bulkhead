...
required=(
	...
	COMPATIBILITY.md example_test.go docs/README.md docs/api.md
	docs/architecture.md docs/assurance.md docs/composition.md docs/faq.md
	docs/kubernetes.md docs/migration.md docs/operations.md
	docs/performance.md docs/security.md
)
...
if grep -En '(, _ :=|_ = .*\.(Close|Release)\(\)|defer .*\.Release\(\))' \
